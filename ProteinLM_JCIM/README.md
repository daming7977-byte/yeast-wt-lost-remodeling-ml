...
```bash
./run_reproduction.sh main
./run_reproduction.sh robustness
./run_reproduction.sh figures
```

Use `./run_reproduction.sh all` to run all three stages. See `REPRODUCTION_MANIFEST.md` for the analysis map and interpretation notes.
