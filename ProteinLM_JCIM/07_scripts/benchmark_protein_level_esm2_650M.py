...
ESM = Path("ProteinLM_JCIM/01_embeddings/esm2_650M_full_protein_embeddings.tsv")
OUT = Path("ProteinLM_JCIM/05_results/protein_level_esm2_650M_benchmark.tsv")

protein = pd.read_csv(PROTEIN, sep="\t")
esm = pd.read_csv(ESM, sep="\t")
...
esm_features = [c for c in df.columns if c.startswith("esm2_650M_") or c.startswith("esm2_full_")]

print("Traditional features:", len(traditional_features))
print("ESM features:", len(esm_features))

feature_sets = {
    "traditional_safe": traditional_features,
    "esm2_650M_full_protein": esm_features,
    "traditional_plus_esm2_650M": traditional_features + esm_features,
}
